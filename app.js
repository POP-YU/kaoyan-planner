...
const septemberContinuation = {
  ...
  '2026-09-30':{
    ...
    majorGate:[
     ...
     monthGate:'登记880小测正确数/三类错因、436需回补的章-节-内容单元（一轮213个按5个/天、9/3起账，预计10/25账收口）、英语重复错因、近7天23:30前关灯天数；若880低于4/6、436任一章只记标题或计算空白、英语同类错重复2次、关灯少于5/7天，10月首个对应块先回补'
  }
};
...
// 9/7）开始：今天背第4–8个，明天第9–13个，依此推进。
const currentBaselineDate = '2026-09-03';
const majorBaseline = Object.freeze({completedUnits:3,dailyNewUnits:5,label:'内容单元'});
...
const routeData = [
  {dates:'9月2日—9月13日',title:'高强度起步 · 先把今天做实',desc:'9/2–9/5连续四天没有执行，2026-09-06作为实际第1天，账面任务整体顺延4天消化，不把未完成当完成。880改按带刷计划表逐日跳选题号（2026-09-07中午拍板；实际9/8开刷，必做828题9/8–10/17完成，881个题号已与原表逐题核对一致）；课内为纯 FHSU 课程不占用，880 带刷题单写在课余/晚间数学格，436出声背放家里晚间。每天18:05回家：到家先充电+挂下载，晚自习在家三段走。线代第1天先收尾第2章（2.8@34%），概率从第1讲开始，英语一篇一闭环；436快速多轮，每个学习日新增5个内容单元。',check:'验收：数学闭卷+错因；436按资料顺序输出；睡眠守住00:00最晚边界',color:'#5572b8',tint:'#e8eefb'},
  ...
  {dates:'10月1日—10月31日',title:'带刷收尾 + 真题入口',desc:'880带刷计划收尾：10/17前完成必做828题，10/18–10/20选择做37题集中处理，10/21特难题16题收尾（逐日题单见课表“880 带刷”格）。436进入二轮滚动输出（5个/天继续），英语阅读错因保温，政治刷选择题。',check:'9月30日只是测量与校准：现在就执行；月底登记实际数据，按剩余题单、可用分钟、订正/回测量重排，不等到9月30日；未达项首块先回补。',color:'#3b9b94',tint:'#e1f3f0'},
...
function progressFor(index, day, type, id='', rawTitle='', dateOverride=''){
  ...
  if(type==='major' && /436/.test(rawTitle||'')){
    ...
    return {
      label:ordinal,
      note:`前三个已背；按资料顺序核对内容单元；未完次日只补3个。${majorIsReview(original)?'本格是回收/输出，不增加新量。':'本格新增5个内容单元（快速多轮，不死磕），先框架后闭卷复述。'}`,
