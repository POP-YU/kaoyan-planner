...
const courseLedger = [
  ...
  {subject:'436 · 背诵笔记',now:'《背诵笔记》结构已逐页核对：背诵篇10章213个编号知识点（p1–75）+ 计算题篇按章重点题（p76–168）；9/10实际第1–3个已背熟，第4个起未完成',week:'背诵和默写排在880前面；9/11不再重复第1–3个，早格直接背第4–6个，中午接第7–8个，下午统一验收第4–8个；周日只回收，没完成就从最早断点顺延，不挤睡眠',done:'按手头资料顺序合书复述；短答/计算完整落笔；按A/B/C记录断点章-节-内容单元'},
...
function applyNewCourseRescueOverride(actualKey,dayIndex,rows,overflow){
  ...
  if(actualKey==='2026-09-11'){
    ...
    const midday=find('12:45','13:50');
    if(midday)Object.assign(midday,{type:'major',title:'436 · 第7–8个新内容单元（第一章） + 短答2题',note:'承接早上的第4–6个，只继续第7–8个，不重复；完成后为下午第4–8个统一闭卷验收做准备。',why:'一天仍是5个新内容单元，但拆成早3个、中午2个，避免早上重复旧内容。',steps:['学习第7–8个','各压答题骨架','完成短答2题'],output:'第7–8个框架+短答2题'});
    const afternoon=find('16:35','17:25');
    if(afternoon)Object.assign(afternoon,{type:'major',title:'436 · 第4–8个新内容单元 · A/B/C闭卷验收',note:'早上已学第4–6个、中午已学第7–8个；这一格统一关书口述和默写，第1–3个不再重复占时间。',why:'背诵和默写必须在880之前收口。',steps:['关书口述4–8','框架默写','登记A/B/C'],output:'第4–8个等级+断点'});
  }
