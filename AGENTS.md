...
| `setAgendaDayOffset()` / `setupDayNavigation()` | 默认今天；向左划看明天、向右划回今天，只在松手后切换且不做跟手位移，按钮是同等兜底 |
...
- 页面默认今天；在 `#daily-agenda` 向左划看明天、向右划回今天，只在松手后切换，不注册 `pointermove` 跟手动画，也可点“今天 / 明天”。顶部“当前安排 / 接下来”始终取真实今天，不把明日预览误标为正在进行。
...
- **2026-09-09 最新课内硬约束（覆盖旧口径）**：营销学、商业政策、财务管理三门 FHSU 课程正常听课且保持纯课程行；报关实务、外贸英文函电、国际贸易实务、形势与政策4等非 FHSU 课时全部通过 `classAgendaRow()` 转成数学刷题格，课内绝不排 436。`allocateBrushPlanAcrossAgenda()` 按课内和课余数学格的分钟数分摊当日 880 总题量；每格有 `brushCount`，全日之和必须严格等于题单数量，不得把同一批题重复计算。
