...
| `currentBaselineDate` / `majorBaseline` / `majorFirstDayCompletedUnits` / `majorSecondDayCompletedUnits` | 436 账面起点与候选上限；9/10完成第1–3个，9/11截至20:16只完成第4个，后续从第5个接续 |
...
| `applyReportedNapOverride()` / `reportedBrushCompleted` / `reportedBrushRemaining` | 9/10 最终实绩覆盖：880完成17/32、剩15；436完成第1–3个；23:15一局游戏后00:00关灯 |
| `newCourseRescueStart` / `applyNewCourseRescueOverride()` / `newCourseRescueFocusFor()` | 9/11–9/17 七天新课抢救：整周数学约60%新课主线、40% 880，并显示每日实际分钟比例 |
| `holidaySprintStart` / `holidaySprintFocusFor()` / `holidaySprintAgenda()` | 学校通知核对后的9/25–10/7连续13天停课冲刺；课程、通勤和BB退出，固定12小时15分核心学习 |
| `prioritizeMemorizationBeforeBrush()` | 常规把436背诵/默写排在课余880之前；9/11晚按用户20:16即时决定例外为20:20先刷、21:45后补第5–6个；非FHSU课内880仍固定 |
| `composeDailyAgenda()` / `brushFocusFor()` / `agendaPosition()` | 最终页面合成、880 活跃题单、当前/下一格定位；课程、生活框架、顺延任务的优先级只在这里收口，schedule 测试会直接检查最终结果 |
| `blackboardFridayBlock()` | 真实星期约束：Blackboard/BB 常规只在周五19:30–20:30；9/11显式覆盖为19:30–20:20，历史账面的homework行全部丢弃 |
...
- **默认直接发布授权（2026-09-10）**：用户以后发送新进度、资料或规则并要求更新本课表时，修改完成且三套测试全绿后，直接提交并推送 `main`，不再另问“是否上传”；发布后仍要核对远程提交与 Pages 实际版本。若测试失败、事实冲突或可能公开本地原始资料/隐私，则停止发布并明确说明。

## 2026-09-10 最新实绩、抢救期与页面规则

- 9/9 以前的数学 880 与 436 完成标记全部作废；实际 9/10 统一起跑：`scheduleLagDays=8`，9/10 消化9/2首账，880显示原9/8首日32题。
- 9/10 最终实绩覆盖计划：880完成17/32、剩15题；436第1–3个已背熟、第4个起未完成；英语和线代2.8也未完成。页面必须区分候选题量、排入时间和实际完成量。
- 9/11–9/17 为七天新课抢救期：数学整周约60%给线代/概率新课及紧跟对应题、约40%给880时间盒。880先做9/10剩15题，再开当天候选题单；跨章节题按P/W/A分流，P前置未学不得当普通错题。
- Blackboard/BB 是真实星期约束，常规只在每周五19:30–20:30出现一次；9/11因用户明确20:20开刷，仅当天缩为19:30–20:20。最终合成层忽略旧模板里所有`homework`行，周六、周日和白天不得出现。
- 页面默认今天；在 `#daily-agenda` 向左划看明天、向右划回今天，只在松手后切换，不注册 `pointermove` 跟手动画，也可点“今天 / 明天”。顶部“当前安排 / 接下来”始终取真实今天，不把明日预览误标为正在进行。
- `composeDailyAgenda()` 是最终渲染的单一入口：真实课程优先于冲突的休息占位；普通在校日固定晚间框架只保留一段 18:05–18:25 回家通勤，并保留 19:10–19:30 洗澡恢复块。9/11是明确例外：周五只有10:10–11:45上午课，11:45直接回家，后面不得再生成第二段18:05通勤。
...
## 当前锚点快照（2026-09-11，以用户最新进展覆盖旧计划）

- **2026-09-12新增的13天全天冲刺（最新日历约束）**：学校通知明确9/25–9/27中秋停课、9/28–9/30运动会停课、10/1–10/7国庆停课，首尾相接共13天。`composeDailyAgenda()`在这13天直接使用`holidaySprintAgenda()`，不得生成课程、课内学习格、通勤或Blackboard。每天核心学习735分钟：数学470、436为145、英语80、政治40；仍保留吃饭、短休、25分钟午休和00:00关灯。9/25必须从9/24登记的真实断点开始，不得猜测届时讲次、题号或436完成序号。

- **实际第 1 天 = 2026-09-10**，9/2 起的账面整体后移 8 天。
- **9/11真实动线与强度**：用户课表截图及口述确认周五只有10:10–11:45外贸英文函电，非FHSU课内整段量化为880，不另设课上休息；11:45直接回家，12:05–12:45在家吃饭或点外卖，12:45–13:10午休。原15:00–16:35整段休息删除，也不再生成18:05第二次回家通勤。当日核心学习排入上限为12小时30分，仍保留吃饭和00:00关灯。
- **436最新实绩与今晚调整（9/11 20:16）**：9/10完成第1–3个；9/11只确认第4个“资产评估十项基本要素”已记住，第5–6个未完成，第7–8个未开始。原PDF显示第4个有十项、第5个有三类、第6个有五个特点且跨页，所以5个/学习日只作候选上限，不再按编号机械等量。今晚20:20–21:45刷880，21:45后分别处理第5、6个，英语薄弱词顺延，00:00关灯不变；未来从最早未完成第5个接续，只有闭卷通过才推进。
- **880**：2026-09-07 中午用户拍板改回**带刷表跳选题号**（推翻同日早间“顺序推进”方案）。
  逐日原题单在 `math880BrushPlan`（44 天，9/8→10/21；必做 828、选择做 37、特难题 16，合计 881，已与原表核对）。9/10首日32题实际完成17题，剩15题进入9/11队首；原10/23只作候选收尾锚点，实际进度按未完成队列顺延。
...
- **436 页面底部执行法（2026-09-11）**：按理解性记忆重做。每天最多5个只是候选上限；短条目可连做，长条目可独占一格。第5个用“做结论/给建议/查质量”区分三类，第6个用“什么时候/什么市场/为何有价值/谁保证中立/结果什么性质”映射五特点。完成证据仍是关书口述与手写骨架，按A/B/C登记；该区必须位于436目录之后、`</main>`之前，不得把熟读或坐满时间写成完成，也不得承诺上岸。
- **9/10 实际进度覆盖（2026-09-10 用户最终报告）**：14:20–18:07为实际睡眠；880最终17/32、剩15；436最终第1–3个已背熟、第4个起未完成；英语与线代2.8未完成。23:15–23:40一局游戏，随后洗漱，00:00关灯。线代2.8放入9/11 08:20–09:50，原03-01与周五19:30 BB不删除。
- **新博主资料与七天抢救（2026-09-10）**：本地资料为19条精读总结+483条视频元数据；视频标题、点赞、个案分数和“稳上岸”承诺都不是用户事实。9/11–9/17整周约60%新课主线/40% 880，以新课时间戳、对应题、880最后题号与P/W/A验收；不得按“每天10小时”机械加时，也不得用一道跨章节题判断整章能力。
