...
- **436 页面底部执行法（2026-09-10）**：按当前“从零开始、非专业、5 个新内容/学习日、周日只回收、课内不背 436”定制。完成证据固定为关书口述与完整手写，按 A/B/C 登记并默认在当天、次日、第3天、第7天回访；若 5 个中至少 3 个为 C，下一 436 格先清欠账，没清完就顺延。该区必须位于 436 目录之后、`</main>` 之前；不得把熟读、划线或坐满时间写成完成，也不得承诺上岸。
- **9/10 实际进度覆盖（2026-09-10 用户实时报告）**：14:20–18:07 为实际睡眠，且用户随后确认 12:45–14:20 的 436 完全未进行；原 436、15:00 课内数学与线代 2.8 均不得算完成。`applyReportedNapOverride()` 把下午 436 标为未进行，今晚 21:40–23:15 从第 1–5 个的学习与框架默写重新开始，23:15–23:30 登记 A/B/C；英语阅读与薄弱词让位并进入下一执行日，00:00 关灯不动。线代 2.8 放入 9/11 08:20–09:50，原 03-01 与周五 19:30 BB 不删除。
- **新博主资料强度校准（2026-09-10）**：本地资料为 19 条精读总结 + 483 条视频元数据；视频标题、点赞、个案分数和“稳上岸”承诺都不是用户事实。只能吸收结构与验收法：上午数学、白天专业课、晚上复盘补弱；10/23 收尾 880 后，真题按“限时集中→仿真作答→错题二测→总结自检”闭环，模拟卷只补漏洞。最终时间轴 9/11–9/23 的核心考研格已经为 9–12 小时/日（FHSU 课程未计），9/10 最新改排为 7 小时 10 分；不得再按“每天 10 小时”机械加时，先看连续 7 天的真实完成证据。
