...
['小于的考研课表','id="daily-agenda"','id="phase-line"','id="live-clock"','id="current-task-title"','id="next-task-title"','id="date-subtitle"','id="show-today"','id="show-tomorrow"','手指从课表中间向右划看明天','kaoyan.mobileconfig','id="app-version"','apple-touch-icon'].forEach((x) => must(html,x,x));
must(html,'考研任务按未完成账顺延','ledger explanation');
must(html,'路线日期是锚点；日任务按实际执行日顺延','phase anchor explanation');
['styles.css?v=reset-0910b','app.js?v=reset-0910b'].forEach((x) => must(html,x,x));
// 2026-09-08：主屏 Web Clip 用户要求"打开即最新版"。sw.js（网络优先、缓存仅离线兜底）
// 已注册；每次发布必须同步 bump index.html 的 sw.js?v= 与 sw.js 内 CACHE 版本。
must(html,"serviceWorker.register('sw.js?v=reset-0910b')",'service worker registration');
must(sw,"kaoyan-reset-0910b",'sw cache version');
...
['@media(max-width:900px)','phase-card','agenda-item','brush-focus','task-check','ledger-table','--font-ui','font-variant-numeric:tabular-nums','contain:layout paint','overflow-wrap:anywhere','.phase-card small{position:static','margin-top:auto','.day-view-switch','touch-action:pan-y','overscroll-behavior-x:contain','.day-agenda-card.is-tomorrow','--swipe-x','.is-dragging','.swipe-ghost','@keyframes swipe-enter-left','prefers-reduced-motion'].forEach((x) => must(css,x,x));
