...
['styles.css?v=reset-0910h','app.js?v=reset-0910h'].forEach((x) => must(html,x,x));
// 2026-09-08：主屏 Web Clip 用户要求"打开即最新版"。sw.js（网络优先、缓存仅离线兜底）
// 已注册；每次发布必须同步 bump index.html 的 sw.js?v= 与 sw.js 内 CACHE 版本。
must(html,"serviceWorker.register('sw.js?v=reset-0910h')",'service worker registration');
must(sw,"kaoyan-reset-0910h",'sw cache version');
...
[
  ...
  '借结构，不抄“每天10小时”',
  '9月11日至23日每天已排约9—12小时核心考研格',
  '9月10日按“下午436未进行并移到今晚”的最新事实改排7小时10分',
  '安排上限，不是完成记录',
  '连续7天记录880实际完成、436 A/B/C、英语错因卡和关灯时间',
  '限时集中 → 仿真作答 → 错题二测 → 总结自检',
