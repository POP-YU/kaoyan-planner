...
['小于的考研课表','id="daily-agenda"','id="phase-line"','id="live-clock"','id="current-task-title"','id="next-task-title"','id="date-subtitle"','kaoyan.mobileconfig','id="app-version"','apple-touch-icon'].forEach((x) => must(html,x,x));
must(html,'考研任务按未完成账顺延','ledger explanation');
must(html,'路线日期是锚点；日任务按实际执行日顺延','phase anchor explanation');
['styles.css?v=sprint-0909b','app.js?v=sprint-0909b'].forEach((x) => must(html,x,x));
// 2026-09-08：主屏 Web Clip 用户要求"打开即最新版"。sw.js（网络优先、缓存仅离线兜底）
// 已注册；每次发布必须同步 bump index.html 的 sw.js?v= 与 sw.js 内 CACHE 版本。
must(html,"serviceWorker.register('sw.js?v=sprint-0909b')",'service worker registration');
must(sw,"kaoyan-sprint-0909b",'sw cache version');
...
['@media(max-width:900px)','phase-card','agenda-item','brush-focus','task-check','ledger-table','--font-ui','font-variant-numeric:tabular-nums','contain:layout paint','overflow-wrap:anywhere','.phase-card small{position:static','margin-top:auto'].forEach((x) => must(css,x,x));
...
['renderTimetable','renderMajorSyllabus','majorRecitationMaterial','majorChapterCumulative','datedBlocks','buildDayAgenda','composeDailyAgenda','brushFocusFor','agendaPosition','rangeStarts','2026-09-02','actualScheduleStartDate','scheduleLagDays','scheduleSourceDate','currentBaselineDate','majorCumulativeForDate','majorNewRangeForDate','strictDateSchedules','breakfastMenu','breakfastFor','probabilityRawDurations','probabilityModules','probabilityLecture1File','linearAlgebraVerifiedLessons','linearAlgebraLessonSlots','futureLinearQueue','futureLinearLessonForDate','taskCheck','验收：','次日或48小时','剩余题单','可用分钟','订正/回测','方浩第1讲（随机事件：概念、关系与运算）','特征值与特征向量','p1–6','章内题数以手头书为准','九月闭卷小测6题','月末门禁','小作文审题','routeData','updateClock','scheduleClock','visibilitychange','stopLiveUpdates','06:00','24:00','肉夹馍','绿豆粥','英语二 · 2010年 Text 1','英语单词 · 新20 + 旧40','按题号顺序推进','880第一章 · 第1–4题（顺序启动）','按题号顺序','math880BrushPlan','brushPlanEntryFor','brushPlanLagDays','880 带刷','880 · 今日战单','教师：','必做828题9/9–10/19完成','881个题号与原表逐章逐题型一致','Blackboard作业 · 周五开放项','数一不排','今日事今日毕','213个编号知识点','矩阵的分块','断点收尾','18:05','夸克挂下载','承前格续做','applyFixedEveningFrame','APP_VERSION'].forEach((x) => must(js,x,x));
