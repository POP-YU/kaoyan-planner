...
if (!newCourseRescueActive('2026-09-11') || !newCourseRescueActive('2026-09-17') || newCourseRescueActive('2026-09-18')) throw new Error('new-course rescue window boundary is wrong');
{
  let newCourseMinutes=0, brushMinutes=0;
  for (let date=new Date('2026-09-11T12:00:00'); date<=new Date('2026-09-17T12:00:00'); date.setDate(date.getDate()+1)) {
    const rows=composeDailyAgenda(new Date(date),rangeIndexFor(date)).data;
    const focus=newCourseRescueFocusFor(new Date(date),rows);
    if (!focus || !focus.newCourseMinutes || !focus.brushMinutes) throw new Error(`rescue split missing on ${date.toISOString().slice(0,10)}`);
    if (!rows.filter(x=>(x.title||'').startsWith('880 带刷')).every(x=>x.note.includes('P/W/A')&&x.note.includes('前置未学'))) throw new Error(`rescue 880 prerequisite triage missing on ${date.toISOString().slice(0,10)}`);
  ...
  if (!sep15.some(x=>x.start==='21:20'&&x.title.includes('矩阵相似 03'))) throw new Error('9/15 must retain the matrix-similarity lesson before any late 880 work');
  const sep17=composeDailyAgenda(new Date('2026-09-17T12:00:00'),rangeIndexFor(new Date('2026-09-17T12:00:00'))).data;
  if (!sep17.some(x=>x.start==='19:50'&&x.title.includes('矩阵相似 04')) || !sep17.some(x=>(x.title||'').startsWith('880 带刷')&&x.start==='22:50')) throw new Error('9/17 evening must put the matrix-similarity lesson before the final 880 time box');
}
{
  ...
  const truthPhase=routeData.find(x=>x.title.includes('真题入口'));
  const paperPhase=routeData.find(x=>x.title.includes('真题套卷'));
...
}
{
  ...
  if (!brush || brush.sourceDate!=='2026-09-08' || brush.count!==32 || brush.actualCompleted!==17 || brush.actualRemaining!==15) throw new Error('9/10 focus must distinguish the 32-question plan from the verified 17 completed and 15 remaining');
  if (!first.some(x=>x.type==='major' && x.title.includes('第1–3个已背熟')&&x.title.includes('第4个起未完成'))) throw new Error('9/10 436 actual result must record units 1-3 learned and the rest unfinished');
  ...
  if (!remaining || remaining.type!=='math' || remaining.brushCount!==22 || remaining.actualBrushCompleted!==17 || !remaining.note.includes('剩15题移到明天')) throw new Error('9/10 evening must show the verified 17/32 result and 15-question carryover');
  if (!first.some(x=>x.start==='12:45'&&x.end==='14:20'&&x.type==='buffer'&&x.title.includes('未进行')&&x.note.includes('第1–3个已背熟'))) throw new Error('9/10 missed afternoon 436 block must remain separate from the later partial result');
  if (!first.some(x=>x.start==='21:40'&&x.end==='23:15'&&x.type==='major'&&x.title.includes('第1–3个已背熟')&&x.title.includes('第4个起未完成'))) throw new Error('9/10 436 block must record the user-reported partial result');
  if (!first.some(x=>x.start==='23:15'&&x.end==='23:40'&&x.type==='free'&&x.title.includes('游戏一局'))) throw new Error('9/10 must contain the requested one-game time box');
  if (!first.some(x=>x.start==='23:40'&&x.end==='24:00'&&x.type==='sleep'&&x.title.includes('00:00关灯'))) throw new Error('one-game time box must still preserve the midnight sleep boundary');
  ...
  if (!first.some(x=>x.start==='21:40'&&x.note.includes('英语阅读与薄弱词今晚未做'))) throw new Error('9/10 partial-result row must truthfully retain displaced English work');
...
if (holidaySprintStart!=='2026-09-25' || holidaySprintEnd!=='2026-10-07') throw new Error('the 13-day all-day sprint must match the verified school closure notice');
if (holidaySprintActive('2026-09-24') || !holidaySprintActive('2026-09-25') || !holidaySprintActive('2026-10-07') || holidaySprintActive('2026-10-08')) throw new Error('13-day sprint boundary is wrong');
{
  let count=0;
  for(let date=new Date('2026-09-25T12:00:00');date<=new Date('2026-10-07T12:00:00');date.setDate(date.getDate()+1)){
    const key=date.toISOString().slice(0,10),{data,overflow}=composeDailyAgenda(new Date(date),rangeIndexFor(date));count++;
    if(overflow.length)throw new Error(`${key} holiday sprint must be self-contained without overflow`);
    if(data[0]?.start!=='06:00'||data.at(-1)?.end!=='24:00')throw new Error(`${key} holiday sprint must cover 06:00-24:00`);
    for(let i=1;i<data.length;i++)if(data[i-1].end!==data[i].start)throw new Error(`${key} holiday sprint gap/overlap ${data[i-1].end}->${data[i].start}`);
    if(data.some(x=>x.type==='course'||x.type==='fhsu'||x.type==='homework'||x.studyClass))throw new Error(`${key} is a school closure day and must not contain classes, class-study, or Blackboard`);
    const core=data.filter(x=>['math','major','english','politics'].includes(x.type)).reduce((sum,x)=>sum+minutes(x.end)-minutes(x.start),0);
    if(core!==735)throw new Error(`${key} holiday sprint must contain exactly 12h15m core study, got ${core} minutes`);
    if(!data.some(x=>x.start==='06:20'&&x.type==='major'&&x.title.includes('最早未完成')))throw new Error(`${key} must put 436 recall before problem solving`);
    if(!data.some(x=>x.start==='14:50'&&x.title.startsWith('880 带刷'))||!data.some(x=>x.start==='18:50'&&x.title.startsWith('880 带刷')))throw new Error(`${key} must keep 880 in the later afternoon/evening boxes`);
    if(!data.filter(x=>x.title.startsWith('880 带刷')).every(x=>x.note.includes('P/W/A')))throw new Error(`${key} 880 blocks must retain prerequisite triage`);
    if(!data.some(x=>x.start==='23:15'&&x.end==='24:00'&&x.type==='sleep'&&x.title.includes('00:00关灯')))throw new Error(`${key} must preserve midnight sleep`);
    const focus=holidaySprintFocusFor(key);
    if(!focus||focus.day!==count)throw new Error(`${key} must expose its day-specific sprint focus`);
  }
  if(count!==13)throw new Error(`school closure sprint must contain exactly 13 days, got ${count}`);
  if(!holidaySprintFocusFor('2026-09-25').label.includes('启动') || !holidaySprintFocusFor('2026-10-07').label.includes('总验收')) throw new Error('13-day sprint must start with a baseline and end with a real evidence review');
}
for (let date=new Date('2026-09-10T12:00:00'); date<=new Date('2026-10-31T12:00:00'); date.setDate(date.getDate()+1)) {
  ...
  const key=date.toISOString().slice(0,10),friday=date.getDay()===5&&!holidaySprintActive(key);
  ...
  if (friday) {
    const expectedEnd=key==='2026-09-11'?'20:20':blackboardFridayEnd;
...
if (!sep10Shifted.includes('第1–3个新内容单元') || !sep10Shifted.includes('2010年 Text 1') || !sep10Shifted.includes('880 带刷（必做 · 32题') || !sep10Shifted.includes('第2章 08 矩阵的分块（断点收尾）')) throw new Error('September 10 first ledger must retain the verified task sources while recording the partial 436 result');
...
if (routeData[0].dates !== '9月10日—9月24日' || !routeData[0].desc.includes('完成17/32题、剩15题') || !routeData[0].desc.includes('确认第4个')) throw new Error('lead-in route must preserve the latest math and unit-4-only 436 result');
if (routeData[1].dates !== '9月25日—10月7日' || !routeData[1].desc.includes('连续13天停课') || !routeData[1].desc.includes('12小时15分核心学习')) throw new Error('phase route must expose the verified 13-day school-closure sprint');
if (!routeData.every(x => x.desc.includes('带刷'))) throw new Error('every phase must reference the brush plan');
if (!routeData[2].check.includes('10月7日') || !routeData[2].check.includes('实际数据')) throw new Error('post-sprint route must be gated by the 13-day evidence');
...
for (let date=new Date('2026-09-09T12:00:00'); date<=new Date('2026-10-23T12:00:00'); date.setDate(date.getDate()+1)) {
  ...
  } else if (holidaySprintActive(key)) {
    if (commutes.length!==0) throw new Error(`school-closure sprint must not show a commute on ${key}`);
...
}
{
  ...
  if (!brush || brush.count!==32 || brush.sourceDate!=='2026-09-08' || brush.scheduledMinutes!==235 || brush.deficitMinutes!==0 || brush.actualCompleted!==17 || brush.actualRemaining!==15) throw new Error('9/10 880 focus must reflect both revised slots and the verified partial result');
}
...
for (let date=new Date('2026-09-10T12:00:00'); date<=new Date('2026-10-23T12:00:00'); date.setDate(date.getDate()+1)) {
  ...
  if(holidaySprintActive(key)){
    if(data.some(x=>x.type==='course'||x.type==='fhsu'||x.studyClass))throw new Error(`school-closure class leaked into holiday sprint: ${key}`);
  }else{
...
for (const [date,expected] of [['2026-09-24',0],['2026-09-25',1],['2026-10-07',1],['2026-10-08',2],['2026-11-08',2],['2026-11-09',3],['2026-12-08',3],['2026-12-09',4]]) if (currentRouteIndex(new Date(`${date}T12:00:00`))!==expected) throw new Error(`phase highlight must follow the actual sprint boundary, wrong on ${date}`);
for (const [date, rows] of Object.entries(strictDateSchedules)) {
